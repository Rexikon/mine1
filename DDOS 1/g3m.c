 ...
 *
 *
 * Usage: geminid [-T -R -U -I -N -s -h -d -p -q -l -t]
 *
 *     -T TCP attack [0:ACK, 1:PSH, 2:RST, 3:SYN]   (no default         )
 *                   [4:URG, 5:FIN, 6:ECE, 7:CWR]
 *     -U UDP attack                                (no options         )
 *     -I ICMP attack                               (no options         )
 *     -R Random TCP attack                         (no options         )
...
usage(const char *argv0)
{
    ...
    printf("\e[1musage\e[0m: %s -T0 -U -h 12.12.12.12 -s 100 -x 65500 -t 1200\n", argv0);
    exit(-1);
}
