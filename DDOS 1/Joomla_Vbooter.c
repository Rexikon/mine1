...
static int do_request(char * packet,const struct line_t * line, const char * payload, const unsigned int payloadLen)
{

    int skt = connect_me(line, 80, 10);
    if(skt == -1)
        return 1;
    static const char packetFormat[] = "GET /%s HTTP/1.1\r\n"
    "Host: %s\r\n"
